...
hwconfig = {
    ...
    # Experimential MT6895 support
    0x1172: chipconfig(
        var1=0xA,
        watchdog=0x1c007000, # Valid (dts)
        uart=0x11001000,  # Valid (dts) Note: dts entry renamed from uart0 to apuart0
        brom_payload_addr=0x100A00,
        da_payload_addr=0x201000,
        pl_payload_addr=0x40200000,
        gcpu_base=0x10050000,  # Invalid? (not found in dts)
        dxcc_base=0x10210000,  # Valid (dts)
        sej_base=0x1000a000,  # Valid (dts)
        cqdma_base=0x10212000,  # Valid (dts)
        ap_dma_mem=0x11300800 + 0x1a0,  # Valid (dts) Note: There is a different dts entry at 0x10220000
        blacklist=[(0x102848, 0x0), (0x00106B60, 0x0)],
        blacklist_count=0x0000000A,
        send_ptr=(0x102888, 0xE79C),
        ctrl_buffer=0x00102A9C,
